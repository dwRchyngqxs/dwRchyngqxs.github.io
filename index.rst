...
Single step reductions are provided by our plugin as a tactic in ltac (ltac2 TBD), and as a reduction in `Eval reduction in term.` (vernacular) / `let ... := eval reduction in term in ...` (ltac).
...
There are also advanced reductions and advanced way of counting occurences for the `delta` (and `match` TODO):
